...
/**********************************************************/
int isLegalVersionTuc( Position * pos,char tiles[2][MAXIMUM_MOVE_SIZE],char color)
{
	int i, j;
	int intex;


	if( color != pos-> turn )	//not this player's turn..
		return FALSE;


	if( !canMove( pos,color ) )	//if that player cannot move, the only legal move is null
	{
		if( tiles[ 0 ][ 0 ] == -1 )
			return TRUE;
		else
	...
	if( tiles[ 0 ][ 0 ] == -1 )	//since we can move, null move is illegal
		return FALSE;


	/* first coordinates must be inside board */
	if( (tiles[ 0 ][ 0 ] < 0 ) || ( tiles[ 0 ][ 0 ] >= BOARD_ROWS) )
		return FALSE;
	if( (tiles[ 1 ][ 0 ] < 0 ) || ( tiles[ 1 ][ 0 ] >= BOARD_COLUMNS ) )
		return FALSE;

	/* piece must be ours */
	if( pos->board[ tiles[ 0 ][ 0 ] ][tiles[ 1 ][ 0 ] ] != color )
		return FALSE;


	/* second coordinates must be inside board */
	if( (tiles[ 0 ][ 1 ] < 0 ) || ( tiles[ 0 ][ 1 ] >= BOARD_ROWS) )
		return FALSE;
	if( ( tiles[ 1 ][ 1 ] < 0 ) || ( tiles[ 1 ][ 1 ] >= BOARD_COLUMNS ) )
		return FALSE;

	/* square must be empty */
	if( (pos->board[ tiles[ 0 ][ 1 ] ][ tiles[ 1 ][ 1 ] ] != EMPTY )&&(pos->board[ tiles[ 0 ][ 1 ] ][ tiles[ 1 ][ 1 ] ] != RTILE))
		return FALSE;

	/* if it seems to be a jump */
	if( ( abs( tiles[ 0 ][ 0 ] - tiles[ 0 ][ 1 ] ) == 2 ) && ( abs( tiles[ 1 ][ 0 ] - tiles[ 1 ][ 1 ] ) == 2 ) )
	{
		intex = 1;

		while( 1 )
		{
			if( !canJumpTo(tiles[ 0 ][ intex - 1 ], tiles[ 1 ][ intex -1 ], color, pos, tiles[ 0 ][ intex ], tiles[ 1 ][ intex ] ) )
				return FALSE;
			...
			if( !canJump(tiles[ 0 ][ intex ],tiles[ 1 ][ intex ], color, pos ) )
			{
				if( tiles[ 0 ][ intex + 1 ] == -1 )
					return TRUE;
				else
					return FALSE;
			}
			...
			if( ( tiles[ 0 ][ intex + 1 ] < 0 ) || ( tiles[ 0 ][ intex + 1 ] >= BOARD_ROWS ) )
				return FALSE;
			if( ( tiles[ 1 ][ intex + 1 ] < 0 ) || ( tiles[ 1 ][ intex + 1 ] >= BOARD_COLUMNS ) )
				return FALSE;
	...
	else if( ( abs( tiles[ 0 ][ 0 ] - tiles[ 0 ][ 1 ] ) == 1 ) && ( abs( tiles[ 1 ][ 0 ] - tiles[ 1 ][ 1 ] ) == 1 ) )
	{
		/*check direction*/
		if( ( color == WHITE ) && ( tiles[ 0 ][ 1 ] - tiles[ 0 ][ 0 ] ) != 1 )
			return FALSE;
		if( (color == BLACK ) && ( tiles[ 0 ][ 1 ] - tiles[ 0 ][ 0 ] ) != -1 )
			return FALSE;
